...
class CallUnits(unittest.TestCase):
    def testcase000(self):
        ...
        fx = fileinfo_check_tests.check_callback(
            sourceinfo.fileinfo.getcaller_linenumber, 0)
    ...
    def testcase002(self):
        ...
        fx = fileinfo_check_tests.check_callback(sourceinfo.fileinfo.getcaller_linenumber, 2)

        sys.path.pop(0)

        self.assertEqual(fx, 53)
        assert sys.path == _s
    ...
    def testcase003(self):
        ...
        if version >= '3.3':  # pragma: no cover
            fxn = fileinfo_check_tests.check_callback(
                sourceinfo.objectinfo.getcaller_name, 4)
        else:
            fxn = fileinfo_check_tests.check_callback(
                sourceinfo.objectinfo.getcaller_name, 3)
        fx = fileinfo_check_tests.check_callback(
            sourceinfo.fileinfo.getcaller_linenumber, 3)
        ...
        if version == '2.6':  # pragma: no cover
            ...
            # depends on platform, may require additional, anyhow keep it as a ref
            assert fx > 300 and fx < 400
        elif version >= '3.3':  # pragma: no cover
            # depends on platform, may require additional, anyhow keep it as a ref
            assert fx > 600 and fx < 700
        else:
            assert False
